from q_settings import standard_init as std
...
TRAIN TEST SPLIT
"""
def norsplit_(X,Y,vTEST_RATIO):
    #X = X.astype('float32')
    #Y = Y.astype('float32')
    x_train = X[int(X.shape[0] * vTEST_RATIO):]
    x_test  = X[:int(X.shape[0] * vTEST_RATIO)]
    y_train = Y[int(X.shape[0] * vTEST_RATIO):]
    y_test  = Y[:int(X.shape[0] * vTEST_RATIO)]
...
PRE PROCESS DATA
"""
def pre_process_(vInput, col_importance):
    ...
    #Train Test Split
    #test1, x_para , test3, y_para = split_(X,Y)
    #x_para = x_para[['Country_Code', 'Business Unit_code','Magnitude_Product_Group_Name','Customer Business Sector']]
    
    #ColImportance 
    X = X[col_importance['Specs'].values]
    ...
    x_column = X.columns
    X = scale_(X)
    X = pd.DataFrame(X, columns= x_column, index= X_copy.index)
    X = X.fillna(0)
    ...
    return x_train, x_test, y_train, y_test, X, Y #, x_para
...
def holts_winter_(x_train, x_test, y_train, y_test,X,Y):
    #Guessed Parameter
    span = 12
    ...
    #Model
    from statsmodels.tsa.api import ExponentialSmoothing
    model = ExponentialSmoothing(y_train, trend='mul',seasonal='mul',
                                seasonal_periods=12)
    model.exog = x_train
    ...
    Actual_df['Sales'].plot(legend=True)
    #Actual_df['y'].plot(legend=True)
    predicted_df['Predicted'].plot(legend=True )
    '''

    
    return y_test, predicted_df#, None
...
def auto_arima_(x_train, x_test, y_train, y_test,X,Y):
    print("...")
    print("AUTO ARIMA")
    ...
    #smodel.summary()
    smodel_fit = smodel.fit(y_train, exogenous = x_train)
    """
    # Cross Validation
    ...
    """
    prediction_values = smodel_fit.predict(n_periods=len(y_test), exogenous = x_test)
...
"""
import pmdarima as pm
def auto_sarima_(x_train, x_test, y_train, y_test,X,Y):
    print("...")
...
LSTM
"""
def lstm_(x_train, x_test, y_train, y_test,X,Y):
    ...
    history = model.fit(x_train2, y_train2 ,epochs=1000, batch_size=50, validation_data=(x_test2, y_test2), verbose=1, shuffle=False)
...
"""
def pre_process_scenario_(vInput, Ext_var, vCalenderCountry, vMagnitudeLevel, vBusinesUnit):
    #Data Charater
    vdata1 = pd.read_csv(vInput)
    vdata1 = vdata1.set_index(pd.DatetimeIndex(vdata1['key']))
    
    # Filtering Data
    #data = df.loc[df['Calendar Country'].isin(var.std.vCalenderCountry) & df['Business Unit'].isin(var.std.vBusinesUnit) & df['Magnitude_Product_Group_Name'].isin(var.std.vMagnitudeLevel)]
    vdata1 = vdata1.loc[vdata1['Calendar Country'].isin(vCalenderCountry)]
    vdata1 = vdata1.loc[vdata1['Business Unit'].isin(vBusinesUnit)]
    vdata1 = vdata1.loc[vdata1['Magnitude_Product_Group_Name'].isin(vMagnitudeLevel)]
    
    vdata1.drop(['key','Calendar Country', 'Business Unit','Magnitude_Product_Group_Name'],axis=1, inplace=True)
    vdata1 = vdata1.resample('M').mean()
    
    
    # X & Y Split
    Y = vdata1[var.std.vTarget]
    X = vdata1.drop(var.std.vTarget,axis=1)
    X = X[Ext_var].dropna()
